CREATE TABLE loan
(
    loan_id INT NOT NULL AUTO_INCREMENT,
    book_id INT NOT NULL,
    patron_id INT NOT NULL,
    rental_date TIMESTAMP NOT NULL DEFAULT (CURRENT_TIMESTAMP()),
    due_date TIMESTAMP NOT NULL DEFAULT (CURRENT_TIMESTAMP() + INTERVAL 14 DAY),
    return_date TIMESTAMP NULL,
    fees_charged DECIMAL(10,2) NOT NULL DEFAULT 0.00,
    fees_paid DECIMAL(10,2) NOT NULL DEFAULT 0.00,
    notes TEXT NULL,
    really_active ENUM('Y') GENERATED ALWAYS AS (IF (return_date IS NULL, 'Y', NULL)) COMMENT 'If null, the book is available to loan but cannot be deleted until fees are paid.',
    PRIMARY KEY (loan_id),
    CONSTRAINT uc_loan_book_id UNIQUE KEY (book_id, really_active),
    KEY idx_loan_patron_id (patron_id),
    CONSTRAINT fk_loan_book FOREIGN KEY (book_id) REFERENCES library_catalog.book (book_id) ON DELETE RESTRICT,
    CONSTRAINT fk_loan_patron FOREIGN KEY (patron_id) REFERENCES library_identity.patron (patron_id) ON DELETE RESTRICT
);
