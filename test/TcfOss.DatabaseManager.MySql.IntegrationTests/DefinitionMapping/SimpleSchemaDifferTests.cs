    ...
    where TFixture : SimpleSchemaDifferFixture<TBuilderEntity, TContainerEntity>
{
    ...
    protected virtual bool ExpectNullableNulltestAlter => true;
    protected virtual bool ExpectNullableNulltestAlterAfterSamplesModify => false;
    ...
    public void Table_Default_And_Explicit_Column_Changes_Are_Detected()
    {
        ...
        if (ExpectNullableNulltestAlter)
        {
            expectedStatements.Insert(2, "ALTER TABLE `nulltest` MODIFY COLUMN `name` VARCHAR(50) NULL");
        }
        if (ExpectNullableNulltestAlterAfterSamplesModify)
        {
            expectedStatements.Insert(3, "ALTER TABLE `nulltest` MODIFY COLUMN `name` VARCHAR(50) NULL");
        }
