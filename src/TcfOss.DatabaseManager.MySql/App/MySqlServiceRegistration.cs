...
public static class MySqlServiceRegistration
{
    public static void RegisterMySqlServices(this IServiceCollection services, MyConfig config)
    {
        ...
        services.AddPooledDbContextFactory<InfoSchemaContext>((_, options) =>
        {
            options.UseMySql(
                ...
                ServerVersion.Create(config.Version, ServerType.MySql),
                mySqlOpts =>
                {
        ...
        services.AddScoped<IRetrieveDatabaseObjects, InfoSchemaRepo>();
